...
/// لوحة الألوان الخاصة بشاشة الديون.
///
/// تحتوي على الـ visual tokens الخاصة بالـ Debt domain فقط
/// (gradients, category colors, risk colors, progress card).
///
/// الألوان العامة (نص عادي، نص ثانوي، white) بتجي من
/// [ColorScheme] بتاع التطبيق، وليست هنا.
@immutable
class DebtPalette extends ThemeExtension<DebtPalette> {
  ...
  /// الوضع الافتراضي (Dark) — متطابق بصرياً مع التصميم الحالي.
  ...
  /// نموذج مبدئي لوضع Light — جاهز للتفعيل لاحقاً.
  /// لا يؤثر على الـ Dark UI الحالي.
  ...
  DebtPalette lerp(ThemeExtension<DebtPalette>? other, double t) {
    ...
    return DebtPalette(
      ...
      summaryBorder:
          Color.lerp(summaryBorder, other.summaryBorder, t)!,
      summaryGlow: Color.lerp(summaryGlow, other.summaryGlow, t)!,
      summaryDivider:
          Color.lerp(summaryDivider, other.summaryDivider, t)!,
      ...
      progressBorder:
          Color.lerp(progressBorder, other.progressBorder, t)!,
      progressAccent:
          Color.lerp(progressAccent, other.progressAccent, t)!,
    );
  }
}

/// Helper للوصول السريع من أي widget.

...
class LedgerStressTestService {
  final TransactionService _txService =
      TransactionService.instance; // ✅ تم التصحيح
  ...
  Future<StressTestResult> runStressTest({
    ...
    for (int i = 0; i < transactionCount; i++) {
      try {
        ...
        if (tx != null) {
          ...
          if (verbose && i % 50 == 0) {
            print(
              "   ✅ Progress: $successCount / $transactionCount transactions created",
            );
          }
        } else {
          failCount++;
    ...
    print(
      "   ⚠️ Duplicate Issues: ${report.duplicateTransactionIssues.length}",
    );
    ...
    if (report.duplicateTransactionIssues.isNotEmpty) {
      ...
      if (report.duplicateTransactionIssues.length > 5) {
        print(
          "   ... and ${report.duplicateTransactionIssues.length - 5} more",
        );
      }
    }
...
class StressTestResult {
  ...
  double get successRate =>
      targetCount > 0 ? (successCount / targetCount) * 100 : 0;
}

...
- [ ] Active-only commitment filter enforced in the production action provider.
- [ ] Completed one-time occurrences never reappear.
- [ ] Missed recurring occurrence policy is implemented and tested.
- [ ] Occurrence lifecycle includes explicit failure/retry semantics.
- [ ] Occurrence completion and rule advancement are idempotent.
...
- [ ] Production provider has direct unit/integration coverage.
- [ ] Duplicate `CommitmentActionProvider` behavior is consolidated.

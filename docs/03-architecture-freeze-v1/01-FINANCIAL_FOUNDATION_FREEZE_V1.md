...
**Status:** Implementation Gate — V1.1 progress in Scheduled Money Gate G
...
**V1.1 progress:** The first two provider lifecycle defects are now addressed in the production provider: only `CommitmentStatus.active` commitments are actionable, and completed current occurrences are suppressed. The duplicate provider implementation is also consolidated behind a compatibility export. Direct production-provider tests were added; full test execution requires the complete Flutter project (including `pubspec.yaml`), which is not present in the supplied source archive.
